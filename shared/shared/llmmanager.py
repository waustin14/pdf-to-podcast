...
class ModelConfig:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "ModelConfig":
        ...
        return cls(
            name=data["name"],
            api_base=data["api_base"] if "api_base" in data else "",
        )
...
class LLMManagerGemini:
    """
    A lightweight and user friendly wrapper over Langchain's ChatGoogleGenerativeAI class. We use this class
    to abstract away all Langchain functionalities including models, async/sync queries,
    structured outputs, types, streaming and more. It also comes with OTEL telemetry out of the box
    for all queries. It is specifically tailored for singular invocations.

    Configs can be overridden by providing a custom config file. 

    Attributes:
        api_key (str): API key for Google Gemini AI Studio endpoint
        telemetry (OpenTelemetryInstrumentation): Telemetry instrumentation instance
        _llm_cache (Dict[str, ChatGoogleGenerativeAI]): Cache of initialized LLM models
        model_configs (Dict[str, ModelConfig]): Model configurations
    ...
    def __init__(
        ...
        """
        Initialize LLMManagerGemini with telemetry.
        ...
        try:
            ...
            self.model_configs = self._load_configurations(config_path)
            logger.info("Successfully initialized LLMManagerGemini")
        except Exception as e:
            logger.error(f"Failed to initialize LLMManagerGemini: {e}")
            raise
    ...
    def get_llm(self, model_key: str) -> ChatGoogleGenerativeAI:
        """Get or create a ChatGoogleGenerativeAI model for the specified model key.
        
        Args:
            model_key (str): Key identifying which model configuration to use
            
        Returns:
            ChatGoogleGenerativeAI: Initialized ChatGoogleGenerativeAI instance
        ...
        if model_key not in self._llm_cache:
            config = self.model_configs[model_key]
            self._llm_cache[model_key] = ChatGoogleGenerativeAI(
                model=config.name,
